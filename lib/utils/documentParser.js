...
class DocumentParser {
  ...
  static extractMatrimonyNotices(lines, data) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        // Detect Marriage Bann type (I, II, III)
        const bannMatch = line.match(/MARRIAGE\s+BANN\s*\(([IV]+)\)/i);
        if (bannMatch) {
            // Save previous notice
        ...
        if (line.includes('The following couples intend to celebrate')) {
            ...
            // Extract group date (handles formats like "August 23rd" or "23rd August")
            ...
            // Add the group notice to the list
            data.matrimonyNotices.push(currentNotice);
            continue;
        }

        // Process group wedding couples with more flexible matching
        if (inGroupWedding && line.trim() !== '') {
            // Skip if this is the group header line we already processed
            if (line.includes('The following couples intend to celebrate')) {
                continue;
            }
            
            // Check if this is a couple line (with or without Roman numerals)
            const coupleMatch = line.match(/(?:^[IVX]+\.\s*)?(.+?)\s+&\s+(.+)$/i);
            if (coupleMatch) {
                const groomName = coupleMatch[1].trim();
                const brideName = coupleMatch[2].trim();
                
                // Create a notice for this couple
                const coupleNotice = {
                    bannType: currentNotice.bannType || 'I',
                    groomName: groomName,
                    brideName: brideName,
                    weddingDate: currentNotice.weddingDate,
                    venue: currentNotice.venue,
                    isGroupWedding: true,
                    groupWeddingDate: currentNotice.weddingDate,
                    groupWeddingVenue: currentNotice.venue
                };
                
                // Add the couple to the main notices list
                data.matrimonyNotices.push(coupleNotice);
                
                // Also add to the group's couples array
                if (currentNotice.couples) {
                    currentNotice.couples.push({
                        groomName: groomName,
                        brideName: brideName
                    });
                }
