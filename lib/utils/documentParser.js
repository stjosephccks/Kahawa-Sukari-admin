...
class DocumentParser {
  ...
  static extractMatrimonyNotices(lines, data) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // Look for matrimony section start
      if (line.match(/MARRIAGE\s+BANN/i)) {
        ...
        // Start new notice (will be filled in subsequent lines)
        currentNotice = {};
        continue;
      }

      // Look for groom name
      if (line && !line.match(/MARRIAGE\s+BANN/i) && !line.match(/S\/o|D\/o|INTENDS|ON\s+\w+DAY|AT\s+/i) && 
          currentNotice !== null && !currentNotice.groomName) {
        currentNotice.groomName = line;
        continue;
      }
      ...
      if (currentNotice) {
        ...
        else if (line.match(/INTENDS\s+TO\s+CELEBRATE/i)) {
          // Skip this line, bride name should be on next line
          continue;
        }
        else if (!currentNotice.brideName && line && 
                 !line.match(/S\/o|D\/o|INTENDS|ON\s+\w+DAY|AT\s+|MARRIAGE/i) && 
                 currentNotice.groomName && currentNotice.groomParents) {
          currentNotice.brideName = line;
        }
