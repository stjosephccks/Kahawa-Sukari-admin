...
const { parseBillRef } = require('./utils/billRefParser');
...
class MpesaService {
  ...
  async getAccessToken(paybillType = 'parish') {
    ...
    if (!credentials.consumerKey || !credentials.consumerSecret) {
      throw new Error(`Missing credentials for ${paybillType}: consumerKey or consumerSecret not set`);
    }
    ...
    try {
      ...
      console.log(`Requesting token for ${paybillType} in ${this.environment} environment`);
      ...
      tokenCache.expiry = Date.now() + (expiresIn * 1000) - 60000; // Expire 1 minute early

      return token;
    } catch (error) {
      console.error(`Error getting access token for ${paybillType}:`, error.response?.data || error.message);
      console.error(`Environment: ${this.environment}`);
      console.error(`Consumer Key set: ${!!credentials.consumerKey}`);
      console.error(`Consumer Secret set: ${!!credentials.consumerSecret}`);
  ...
  async processC2BConfirmation(paymentData) {
    try {
      ...
      // Validate phone number - if it's corrupted (too long), log it
      if (phoneNumber.length > 15) {
        console.error(`Corrupted phone number detected: Raw MSISDN=${MSISDN}, Normalized=${phoneNumber}`);
      }
      ...
      // Parse BillRefNumber for zaka number and month
      let parsedZakaNumber = null;
      let parsedMonth = null;
      let parsedYear = null;
      let parseConfidence = 'none';

      if (BillRefNumber && BillRefNumber.trim()) {
        const parsed = parseBillRef(BillRefNumber);
        parsedZakaNumber = parsed.zakaNumber;
        parsedMonth = parsed.month;
        parsedYear = parsed.year;
        parseConfidence = parsed.confidence;
        console.log(`[BillRefParser] Raw="${BillRefNumber}" → zakaNumber=${parsedZakaNumber}, month=${parsedMonth}, year=${parsedYear}, confidence=${parseConfidence}`);
      }

      const Zaka = require('../models/Zaka').Zaka;

      // Try to match by zaka number from parsed BillRefNumber
      if (parsedZakaNumber) {
        zakaMember = await Zaka.findOne({ zakaNumber: parsedZakaNumber });
        if (zakaMember) matchedBy = 'zakaNumber';
      }

      // If not matched by zaka number, try by phone number (only if phone number is valid)
      if (!zakaMember && phoneNumber.length <= 15) {
        const Zaka = require('../models/Zaka').Zaka;
      ...
      if (!zakaMember) {
        ...
        // Log unmatched payment for manual processing (include parsed hints for admin UI)
        const UnmatchedMpesaPayment = require('../models/UnmatchedMpesaPayment').UnmatchedMpesaPayment;
        try {
          await UnmatchedMpesaPayment.create({
            transID: TransID,
            ...
            paybillSource: paybillSource,
            matchedBy: 'none',
            reason: 'No zaka member found with zaka number or phone number',
            // Parser hints to help admin pre-fill the manual assignment form
            parsedZakaNumber: parsedZakaNumber || null,
            parsedMonth: parsedMonth || null,
            parsedYear: parsedYear || null,
            parseConfidence: parseConfidence,
          });
        } catch (logError) {
          console.error('Error logging unmatched payment:', logError);
        }
      ...
      // 1. Use parsed month from BillRefNumber if available (confidence: high)
      // 2. Otherwise fall back to current calendar month
      const transactionDate = this.parseMpesaDate(TransTime);
      const paymentMonth = parsedMonth || new Date().toLocaleString('default', { month: 'long' });
      const paymentYear  = parsedYear  || new Date().getFullYear();
      const monthSource  = parsedMonth ? `BillRef (${parseConfidence})` : 'current month (fallback)';

      // Duplicate guard: check if payment already recorded for this member + month + year
      const ZakaPayment = require('../models/ZakaPayment').ZakaPayment;
      const duplicate = await ZakaPayment.findOne({
      ...
      if (duplicate) {
        console.warn(`[Duplicate] Payment already recorded for zakaNumber=${zakaMember.zakaNumber}, month=${paymentMonth}, year=${paymentYear}. TransID=${TransID}`);
        const UnmatchedMpesaPayment = require('../models/UnmatchedMpesaPayment').UnmatchedMpesaPayment;
        try {
          await UnmatchedMpesaPayment.create({
            transID: TransID,
            ...
            paybillSource: paybillSource,
            matchedBy: matchedBy,
            reason: `Duplicate payment — ${paymentMonth} ${paymentYear} already recorded for zakaNumber ${zakaMember.zakaNumber}`,
            parsedZakaNumber: zakaMember.zakaNumber,
            parsedMonth: paymentMonth,
            parsedYear: paymentYear,
            parseConfidence: parseConfidence,
          });
        } catch (logError) {
          console.error('Error logging duplicate payment:', logError);
        }
        return { ResultCode: 0, ResultDesc: 'Duplicate payment queued for review' };
      }
      ...
      const payment = await ZakaPayment.create({
        ...
        notes: `Matched by ${matchedBy} | Month: ${paymentMonth} ${paymentYear} from ${monthSource} | Paybill: ${paybillSource}`
      });

      // Send SMS confirmation — pass the M-Pesa sender's phone so we always
      // notify the number that actually made the transaction
      await this.sendPaymentConfirmation(zakaMember, payment, paymentMonth, paymentYear, phoneNumber);
   ...
   * @param {Object} zakaMember     - The matched Zaka document
   * @param {Object} payment        - The created ZakaPayment document
   * @param {string} month          - Payment month (e.g. 'September')
   * @param {number} year           - Payment year (e.g. 2024)
   * @param {string} mpesaPhone     - The phone number that actually sent the M-Pesa (guaranteed active)
   */
  async sendPaymentConfirmation(zakaMember, payment, month, year, mpesaPhone) {
    const SmsLog = require('../models/SmsLog').SmsLog;
    const ZakaPayment = require('../models/ZakaPayment').ZakaPayment;

    // Pick the best SMS target:
    // 1. The M-Pesa sender's number (we KNOW it's live, provided it's not a masked hash > 15 chars)
    // 2. Fall back to mobileNumber on the member record
    // 3. Last resort: mobileNumber2
    const isValidMpesaPhone = mpesaPhone && mpesaPhone.length <= 15;
    const smsTarget =
      (isValidMpesaPhone ? mpesaPhone : null) ||
      zakaMember.mobileNumber ||
      zakaMember.mobileNumber2;

    if (!smsTarget) {
      console.warn(`[SMS] No phone number available for zakaNumber=${zakaMember.zakaNumber}. Skipping SMS.`);
      await ZakaPayment.findByIdAndUpdate(payment._id, {
        smsSent: false,
        smsStatus: 'failed',
        smsError: 'No phone number available for member'
      });
      return;
    }

    try {
      const smsService = require('./smsService').default;
      ...
      // Send SMS — note: smsService exposes sendSingleSMS (not sendSMS)
      const result = await smsService.sendSingleSMS(smsTarget, message);
      ...
      try {
        await SmsLog.create({
          recipient: smsTarget || zakaMember.mobileNumber || 'unknown',
          recipientName: zakaMember.fullName,
          zakaNumber: zakaMember.zakaNumber,
