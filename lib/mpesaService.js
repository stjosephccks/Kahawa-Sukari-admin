  ...
  async processC2BConfirmation(paymentData) {
    try {
      ...
      if (BillRefNumber && BillRefNumber.trim()) {
        // Try to match by zaka number - extract just the number part (before any text)
        const zakaNumberMatch = BillRefNumber.trim().match(/^(\d+)/);
        const zakaNumber = zakaNumberMatch ? zakaNumberMatch[1] : BillRefNumber.trim();

        const Zaka = require('../models/Zaka').Zaka;
        zakaMember = await Zaka.findOne({ zakaNumber: zakaNumber });
        if (zakaMember) {
      ...
      if (!zakaMember) {
        ...
        // Log unmatched payment for manual processing
